...
//
	...
	*/
	...
	char str[100];
	sprintf(str, "temp/final%d.jpg",camid);
...
void CameraCaliberation::GetImages(unsigned short int camid)
{
	...
	for(;;)
	{
		...
		if(key == 's')
		{
            sprintf(imagename, "temp/cam2/%s%d.jpeg", FileName, ++NumberofBoards);
    ...
    ImageSize = cv::Size(image.rows, image.cols);
	camera.release();
	image.release();
}
...
void CameraCaliberation::ExtractPointsFromImageFiles()
{
	...
	{
		//sprintf(str,"data/camera/caliberation/patterns/pattern%d.jpg", i);
        sprintf(str, "temp/cam2/%s%d.jpeg", FileName, i+1);
        ...
        ImageSize = cv::Size(img.rows, img.cols);
        ...
        sprintf(str, "temp/detect2/detected%d.jpg",i+1);
		cv::imwrite(str, img);
...
void CameraCaliberation::SaveCaliberationParameters()
{
    ...
    outputfile.open("results/cam2/intrinsic2.txt");
    ...
    outputfile.open("results/cam2/distortion_coeffs2.txt");
	for(i=0; i<DistortionCoefficient.rows; ++i)
	{
		for(j=0; j<DistortionCoefficient.cols; ++j)
    ...
    outputfile.open("results/cam2/rotation2.txt");
    ...
    outputfile.open("results/cam2/translation2.txt");
    ...
    outputfile.open("results/cam2/objectpt2.txt");
    ...
    outputfile.open("results/cam2/imagept2.txt");
